...
pub fn draw_nyaa<B: Backend>(f: &mut Frame<B>, area: Rect, block: Block<'_>, app: &mut App) {
    let entries: Vec<ListItem> = app
        ...
        .items
        .iter()
        .map(|x| ListItem::new(vec![text::Line::from(vec![Span::raw(x.name.to_string())])]))
        .collect();
    ...
    if let Some(pos) = app.nyaa_entries.state.selected() {
...
pub fn draw_add_download<B: Backend>(
    ...
    let info = vec![
        ...
        text::Line::from(vec![
            ...
            Span::styled(
                app.nyaa_entries.items[pos].name.to_string(),
                Style::default().fg(Color::LightBlue),
            ),
            Span::raw("?"),
        ]),
